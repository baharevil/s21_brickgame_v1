...
int init_game_windows(game_windows_t *windows) {
  ...
  // main_win
  init_win(stdscr, &(windows->main_win), (win_data_t){
    .start_x = 0, .start_y = 0,
    .hight = main_min_higth, .min_hight = main_min_higth,
    .width = main_min_width, .min_width = main_min_width
  }, TRUE);

  // game_win
  init_win(windows->main_win.win, &(windows->game_win), (win_data_t){
    .start_x = 1, .start_y = 1,
    .hight = game_min_higth, .min_hight = game_min_higth,
    .width = game_min_width, .min_width = game_min_width
  }, TRUE);

  // stat_win
  init_win(windows->main_win.win, &(windows->stat_win), (win_data_t){
    .start_x = 1 + game_min_width, .start_y = 1,
    .hight = game_min_higth, .min_hight = game_min_higth,
    .width = game_min_width, .min_width = game_min_width
  }, TRUE);
  ...
  for (int i = 0; i < count_support_win; i++) {
    init_win(windows->stat_win.win, &windows->stats_windows.windows[i].window, 
      (win_data_t){
        .start_x = 1, .start_y = 1 + i * support_min_higth,
        .hight = support_min_higth, .min_hight = support_min_higth,
        .width = support_min_width, .min_width = support_min_width
      }, TRUE);
    windows->stats_windows.windows[i].show = TRUE;
    windows->stats_windows.windows[i].label = labels[i];
    mvwprintw(windows->stats_windows.windows[i].window.win, 1, 1, "%s", labels[i]);
